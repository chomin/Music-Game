...
//  音ゲー（仮）
//
//  Created by Kohei Nakai on 2018/08/29.
...
class Reader {
    ...
    static func readHeadContents(fileName: String) throws -> Music {
        
        var music = Music()
        
        // 譜面データファイルを一行ごとに配列で保持
        let bmsData = try Reader.readFile(fileName: fileName)
        ...
        let headerInstructionTable: [String: (String) -> ()] = [
            "GENRE":     { value in music.genre     = value },
            "TITLE":     { value in music.title     = value },
            "ARTIST":    { value in music.artist    = value },
            "VIDEOID":   { value in music.videoID   = value },
            "VIDEOID2":  { value in music.videoID   = value },
            "BPM":       { value in if let num = Double(value) { music.BPMs = [(num, 0.0)] } },
            "PLAYLEVEL": { value in if let num = Int(value)    { music.playLevel = num } },
            "VOLWAV":    { value in if let num = Int(value)    { music.volWav = num } },
            "LANE":      { value in if let num = Int(value)    { music.laneNum = num } }
        ]
