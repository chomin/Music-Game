    ...
    @objc func didTapPlayButton(_ sender : UIButton) {
        ...
        let moveToGameScene = {
            ...
            case self.playButton:
                scene = self.autoPlaySwitch.isOn ? AutoPlayScene(size: (self.view?.bounds.size)!, setting: self.setting, header: self.selectedHeader) : GameScene(size: (self.view?.bounds.size)!, setting: self.setting, header: self.selectedHeader)
            case self.offsetButton:
                scene = OffsetScene(size: (self.view?.bounds.size)!, setting: self.setting, header: self.selectedHeader)
            default: break
            }
            let skView = self.view as SKView?    // このviewはGameViewControllerのskView2
            skView?.showsFPS = true
